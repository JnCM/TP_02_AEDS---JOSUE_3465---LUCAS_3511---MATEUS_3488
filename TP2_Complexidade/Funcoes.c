...
void permuta(int X, int *Vetor, int inf, int sup, int **Matriz, int *Melhor_Cam, int *Menor_Dist, int *cont){
	if(inf == sup){
    int Distancia, i;
    ...
    printf("%d ", X);
      ...
      *cont = 1;
	  ...
	  int i;
		for(i = inf; i <= sup; i++){
      troca(Vetor, inf, i);
      permuta(X, Vetor, inf+1, sup, Matriz, Melhor_Cam, Menor_Dist, cont);
      troca(Vetor, inf, i);
		}
	}
}
